...
function checkName(){
	...
	if (!patrn.test(name)) 
	{  	
		divname.innerHTML='<font id="tt" class="tips_false">2-25位长度</font>';
		return false;
	}else{  
		$.get("checkName?name="+name+"", function(msg){
			if(msg=="success"){
				divname.innerHTML='<font id="tt" class="tips_false">角色已存在</font>';
			}else{
				divname.innerHTML='<font id="tt" class="tips_true">正确</font>';
			}
		});
		if($("#tt").hasClass("tips_true")){
			return true;
		}else{
			return false;
		}
